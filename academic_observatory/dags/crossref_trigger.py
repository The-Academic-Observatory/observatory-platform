...
from release_util import CrossrefRelease as ReleaseUtil
...
def create_url_debug():
    ...
    snapshot_list += 'https://api.crossref.org/snapshots/monthly/3000/01/all.json.tar.gz' + ', '
...
def create_url_list():
    ...
    # Loop through years and months from latest to oldest
    for year in range(int(datetime.today().strftime("%Y")), 2018 -1, -1):
        for month in range(12,1 -1, -1):
            snapshot_url = f"{os.path.join(ReleaseUtil.host,str(year),f'{month:02d}','all.json.tar.gz')}"
            response = requests.head(snapshot_url)
            if response:
