...
module "elasticsearch-logins" {
  for_each = var.elasticsearch_logins
  source = "./secret"
  secret_id = "elasticsearch-${each.key}"
  secret_data = each.value
  service_account_email = google_service_account.api-endpoint_service_account.email
  depends_on = [google_project_service.services]
}
  ...
  template {
    spec {
      containers {
        ...
        env {
          name = "ES_USERNAME"
          value = "sm://${var.google_cloud.project_id}/elasticsearch-username"
        }
        env {
          name = "ES_PASSWORD"
          value = "sm://${var.google_cloud.project_id}/elasticsearch-password"
        }
        env {
          name = "ES_ADDRESS"
          value = "sm://${var.google_cloud.project_id}/elasticsearch-address"
        }
      }
      service_account_name = google_service_account.api-endpoint_service_account.email
    }
  }
  depends_on = [google_endpoints_service.api, module.elasticsearch-logins]
}
