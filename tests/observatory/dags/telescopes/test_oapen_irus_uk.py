...
from observatory.dags.telescopes.oapen_irus_uk import (OapenIrusUkRelease, OapenIrusUkTelescope)
...
from observatory.platform.utils.test_utils import ObservatoryEnvironment, ObservatoryTestCase, module_file_path, test_fixtures_path
from observatory.dags.telescopes.oapen_irus_uk import cloud_function_exists, create_cloud_function, call_cloud_function
...
class TestOapenIrusUk(ObservatoryTestCase):
    ...
    def test_telescope(self, mock_authorized_session, mock_account_credentials, mock_build):
        ...
        http = HttpMockSequence([({'status': '200'}, json.dumps({'functions': [{'name': f'projects/{telescope.oapen_project_id}/locations/{telescope.function_region}/functions/{telescope.function_name}'}]})),
                                 ({'status': '200'}, json.dumps({'name': 'operations/d29ya2Zsb3dzLWRldi91cy1jZW50cmFsMS9vYXBlbl9hY2Nlc3Nfc3RhdHMvWnlmSEdWZDBHTGc', 'done': True, 'response': {'message': 'response'}}))
                                 ])
    ...
    def test_cloud_function_exists(self):
        ...
        :return: None.
        """
        http = HttpMockSequence([({'status': '200'}, json.dumps({'functions': [{'name': 'projects/project-id/locations/us-central1/functions/function-1'}]})),
                                 ({'status': '200'}, json.dumps({'functions': [{'name': 'projects/project-id/locations/us-central1/functions/function-2'}]}))])
    ...
    def test_create_cloud_function(self):
        ...
        http=HttpMockSequence([({'status': '200'}, json.dumps({'name': 'operations/d29ya2Zsb3dzLWRldi91cy1jZW50cmFsMS9vYXBlbl9hY2Nlc3Nfc3RhdHMvWnlmSEdWZDBHTGc'})),
                               ({'status': '200'}, json.dumps({'name': 'operations/d29ya2Zsb3dzLWRldi91cy1jZW50cmFsMS9vYXBlbl9hY2Nlc3Nfc3RhdHMvWnlmSEdWZDBHTGc',
                                                               'done': True,
                                                               'response': {'message': 'response'}}))
                               ])
        service = build('cloudfunctions', 'v1', http=http)
        success, msg = create_cloud_function(service, location, full_name, source_bucket, blob_name, max_active_runs, update=False)
        ...
        service = build('cloudfunctions', 'v1', http=http)
        success, msg = create_cloud_function(service, location, full_name, source_bucket, blob_name, max_active_runs, update=True)
        self.assertTrue(success)
        self.assertDictEqual({'message': 'response'}, msg)
