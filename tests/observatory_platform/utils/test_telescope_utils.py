...
class TestScheduleOptimiser(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.historic_counts_leading_zeros = [
            PeriodCount(SchedulePeriod(start=pendulum.date(1000, 1, 1), end=pendulum.date(1000, 1, 1)), 0),
            PeriodCount(SchedulePeriod(start=pendulum.date(1000, 1, 1), end=pendulum.date(1000, 1, 1)), 0),
            PeriodCount(SchedulePeriod(start=pendulum.date(1000, 1, 1), end=pendulum.date(1000, 1, 1)), 0),
        ...
        self.historic_counts_leading_zeros2 = [
            PeriodCount(SchedulePeriod(start=pendulum.date(1000, 1, 1), end=pendulum.date(1000, 1, 1)), 0),
            PeriodCount(SchedulePeriod(start=pendulum.date(1000, 1, 1), end=pendulum.date(1000, 1, 1)), 0),
        ]

        self.historic_counts_case1 = [
    ...
    def test_optimise(self):
        """ Test API minimisation code. """

        max_per_call = 2
        max_per_query = 10

        schedule, min_calls = ScheduleOptimiser.optimise(max_per_call, max_per_query, self.historic_counts_leading_zeros)
        ...
        schedule, min_calls = ScheduleOptimiser.optimise(max_per_call, max_per_query, self.historic_counts_leading_zeros2)
        self.assertTrue(len(schedule), 1)
        self.assertEqual(min_calls, 0)

        schedule, min_calls = ScheduleOptimiser.optimise(max_per_call, max_per_query, self.historic_counts_trivial)
        self.assertTrue(len(schedule), 1)
        self.assertEqual(min_calls, 3)
        self.assertEqual(len(schedule), 1)

        schedule, min_calls = ScheduleOptimiser.optimise(max_per_call, max_per_query, self.historic_counts_case1)
        self.assertTrue(len(schedule), 2)
        self.assertEqual(min_calls, 8)

        schedule, min_calls = ScheduleOptimiser.optimise(max_per_call, max_per_query, self.historic_counts_case2)
        ...
        schedule, min_calls = ScheduleOptimiser.optimise(max_per_call, max_per_query, self.historic_counts_case3)
