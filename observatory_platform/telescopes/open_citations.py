...
from observatory_platform.utils.config_utils import AirflowVar, SubFolder, telescope_path, check_variables
from observatory_platform.utils.config_utils import find_schema, schema_path
...
def list_open_citations_releases(start_date: Pendulum = None,
    ...
    for version in versions:
        ...
        response = retry_session().get(version_url, timeout=timeout, headers={
...
def fetch_open_citations_versions():
    ...
    response = retry_session().get(url, timeout=30, headers={
        'Accept-encoding': 'gzip'
    })
