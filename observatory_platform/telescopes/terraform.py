...
def get_workspace_id() -> str:
    ...
    :return: workspace id
    """
    token = BaseHook.get_connection(AirflowConn.terraform.get()).password
    terraform_api = TerraformApi(token)

    # Get organization
    organization = Variable.get(AirflowVar.terraform_organization.get())

    # Get workspace
    prefix = Variable.get(AirflowVar.terraform_prefix.get())
...
def check_success_runs(dag_id: str, execution_dates: list) -> bool:
    ...
    for date in execution_dates:
        ...
        for dag_run in dag_runs:
            logging.info(f'id: {dag_run.dag_id}, start date: {dag_run.start_date}, execution date: '
...
class TerraformTelescope:
    ...
    def check_dependencies():
        ...
        vars_valid = check_variables(AirflowVar.project_id.get(), AirflowVar.terraform_organization.get(),
                                     AirflowVar.terraform_prefix.get())
        conns_valid = check_connections(AirflowConn.terraform.get())
    ...
    def get_variable_create(**kwargs) -> bool:
        ...
        for var in variables:
            ...
            if key == TerraformTelescope.TERRAFORM_CREATE_VARIABLE:
        ...
        vm_is_on = eval(value.capitalize())
        if (kwargs["dag_run"].dag_id == TerraformTelescope.DAG_ID_CREATE_VM and vm_is_on) or \
                (kwargs["dag_run"].dag_id == TerraformTelescope.DAG_ID_DESTROY_VM and not vm_is_on):
    ...
    def update_terraform_variable(**kwargs):
        ...
        :return: None
        """
        if kwargs["dag_run"].dag_id == TerraformTelescope.DAG_ID_CREATE_VM:
            value = 'true'
        else:
            value = 'false'
        token = BaseHook.get_connection(AirflowConn.terraform.get()).password
        ...
        # Add variable to workspace
        workspace_id = get_workspace_id()
        terraform_variable = TerraformApi.create_var_attributes(TerraformTelescope.TERRAFORM_CREATE_VARIABLE, value,
                                                                'terraform')
        workspace_vars = terraform_api.list_workspace_variables(workspace_id)
        var_attributes = next(var for var in workspace_vars if var['attributes']['key'] ==
                              TerraformTelescope.TERRAFORM_CREATE_VARIABLE)
        var_id = var_attributes['id']
        terraform_api.update_workspace_variable(terraform_variable, var_id, workspace_id)
    ...
    def terraform_run(**kwargs):
        ...
        if kwargs["dag_run"].dag_id == TerraformTelescope.DAG_ID_CREATE_VM:
            prev_start_time_vm = ti.xcom_pull(key=TerraformTelescope.XCOM_START_TIME_VM,
                                              task_ids=TerraformTelescope.TASK_ID_RUN,
                                              dag_id=TerraformTelescope.DAG_ID_CREATE_VM, include_prior_dates=True)
            ti.xcom_push(TerraformTelescope.XCOM_PREV_START_TIME_VM, prev_start_time_vm)
            ti.xcom_push(TerraformTelescope.XCOM_START_TIME_VM, ti.start_date)
        if kwargs["dag_run"].dag_id == TerraformTelescope.DAG_ID_DESTROY_VM:
            ti.xcom_push(TerraformTelescope.XCOM_DESTROY_TIME_VM, ti.start_date)

        token = BaseHook.get_connection(AirflowConn.terraform.get()).password
        terraform_api = TerraformApi(token)

        target_addrs = TerraformTelescope.TERRAFORM_MODULE_WORKER_VM
        workspace_id = get_workspace_id()
        ...
        # Push run id
        ti.xcom_push(TerraformTelescope.XCOM_TERRAFORM_RUN_ID, run_id)
    ...
    def check_terraform_run_status(**kwargs):
        ...
        If the run status is not successful and the environment isn't dev a warning message will be sent to a slack
        channel.
        :param kwargs: the context passed from the PythonOperator. See
        https://airflow.apache.org/docs/stable/macros-ref.html for a list of the keyword arguments that are passed to
        this argument.
        ...
        """
        ti: TaskInstance = kwargs['ti']
        run_id = ti.xcom_pull(key=TerraformTelescope.XCOM_TERRAFORM_RUN_ID, task_ids=TerraformTelescope.TASK_ID_RUN)

        token = BaseHook.get_connection(AirflowConn.terraform.get()).password
        terraform_api = TerraformApi(token)

        run_status = None
        while run_status not in ['planned_and_finished', 'applied', 'errored', 'discarded', 'canceled',
                                 'force_canceled']:
        ...
        if run_status not in ['planned_and_finished', 'applied']:
            environment = Variable.get(AirflowVar.environment.get())
    ...
    def check_success_dags(**kwargs) -> str:
        ...
        ti: TaskInstance = kwargs['ti']
        prev_start_time_vm = ti.xcom_pull(key=TerraformTelescope.XCOM_PREV_START_TIME_VM,
                                          task_ids=TerraformTelescope.TASK_ID_RUN,
                                          dag_id=TerraformTelescope.DAG_ID_CREATE_VM, include_prior_dates=True)
        start_time_vm = ti.xcom_pull(key=TerraformTelescope.XCOM_START_TIME_VM,
                                     task_ids=TerraformTelescope.TASK_ID_RUN,
                                     dag_id=TerraformTelescope.DAG_ID_CREATE_VM, include_prior_dates=True)
        destroy_time_vm = ti.xcom_pull(key=TerraformTelescope.XCOM_DESTROY_TIME_VM,
                                       task_ids=TerraformTelescope.TASK_ID_RUN,
                                       dag_id=TerraformTelescope.DAG_ID_DESTROY_VM, include_prior_dates=True)
        logging.info(f'prev_start_time_vm: {prev_start_time_vm}, start_time_vm: {start_time_vm}, '
                     f'destroy_time_vm: {destroy_time_vm}\n')

        for dag_id in TerraformTelescope.DAG_IDS_WATCH_LIST:
            dagbag = DagBag()
            ...
            if destroy_time_vm:
                if start_time_vm < destroy_time_vm:
                    ...
                    logging.warning('start_time_vm is before destroy_time_vm. Perhaps the vm was turned on '
        ...
        if destroy_worker_vm:
            return TerraformTelescope.TASK_ID_VAR_UPDATE
        else:
            return TerraformTelescope.TASK_ID_VM_RUNTIME
    ...
    def check_runtime_vm(**kwargs):
        ...
        the environment isn't dev and a message hasn't been send already in the last x hours.
        :param kwargs: the context passed from the PythonOperator. See
        https://airflow.apache.org/docs/stable/macros-ref.html for a list of the keyword arguments that are passed to
        this argument.
        :return: None
        """
        ti: TaskInstance = kwargs['ti']
        last_warning_time = ti.xcom_pull(key=TerraformTelescope.XCOM_WARNING_TIME,
                                         task_ids=TerraformTelescope.TASK_ID_VM_RUNTIME,
                                         dag_id=TerraformTelescope.DAG_ID_DESTROY_VM, include_prior_dates=True)
        start_time_vm = ti.xcom_pull(key=TerraformTelescope.XCOM_START_TIME_VM,
                                     task_ids=TerraformTelescope.TASK_ID_RUN,
                                     dag_id=TerraformTelescope.DAG_ID_CREATE_VM, include_prior_dates=True)
        ...
        if start_time_vm:
            ...
            logging.info(f'Start time VM: {start_time_vm}, hours passed since start time: {hours_on}, warning limit: '
                         f'{TerraformTelescope.VM_RUNTIME_H_WARNING}')
            ...
            if hours_on > TerraformTelescope.VM_RUNTIME_H_WARNING:
                #  check if no warning was sent before or last time was longer ago than warning frequency
                if not hours_since_warning or hours_since_warning > TerraformTelescope.WARNING_FREQUENCY_H:
                    comments = f'Worker VM has been on since {start_time_vm}. No. hours passed since then: {hours_on}.'\
                               f' Warning limit: {TerraformTelescope.VM_RUNTIME_H_WARNING}H'
                    slack_hook = create_slack_webhook(comments, **kwargs)
                    ...
                    ti.xcom_push(TerraformTelescope.XCOM_WARNING_TIME, ti.start_date)
        else:
