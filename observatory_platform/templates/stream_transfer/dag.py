...
with DAG(dag_id=ExampleTelescope.telescope.dag_id, schedule_interval=ExampleTelescope.telescope.schedule_interval,
         default_args=default_args, catchup=False, max_active_runs=1, doc_md=telescope.__doc__) as dag:
    ...
    transfer = ShortCircuitOperator(
        task_id=ExampleTelescope.TASK_ID_TRANSFER,
        python_callable=ExampleTelescope.transfer,
        provide_context=True,
        queue=ExampleTelescope.telescope.queue,
        retries=ExampleTelescope.telescope.max_retries
    )

    # Download transferred data to disk
    download_transferred = PythonOperator(
        task_id=ExampleTelescope.TASK_ID_DOWNLOAD_TRANSFERRED,
        python_callable=ExampleTelescope.download_transferred,
        provide_context=True,
        queue=ExampleTelescope.telescope.queue,
        retries=ExampleTelescope.telescope.max_retries
    )
    ...
    check >> create_release >> transfer >> download_transferred >> transform >> upload_transformed >> bq_load_partition \
    >> bq_delete_old >> bq_append_new >> cleanup
