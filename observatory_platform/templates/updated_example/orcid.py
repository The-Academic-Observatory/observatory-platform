...
class OrcidRelease(StreamRelease):
    ...
    def make_release(**kwargs) -> 'OrcidRelease':
        ...
        start_date, end_date, first_release = ti.xcom_pull(key=orcid.RELEASE_INFO,
...
orcid_subdags.add_setup_task(before_subdag)
...
orcid = StreamTelescope(release_cls=OrcidRelease, dag_id='orcid_singledag', subdag_ids=[], queue='default',
    ...
    ], transform_filenames=['orcid_table1'], merge_partition_fields=['merge_field1'], updated_date_fields=[
        'update_field1'], bq_merge_days=1)
...
orcid.add_load_chain(orcid.make_operators([orcid.bq_load_partition_task, orcid.bq_delete_old_task,
                                           orcid.bq_append_new_task]))
