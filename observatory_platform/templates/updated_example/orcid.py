...
class OrcidTelescope(StreamTelescope):
    ...
    def download(self, release: OrcidRelease) -> bool:
        print(release)
        print('download release', release.lambda_path, release.download_path)
        return True

    def transfer(self, release: OrcidRelease) -> bool:
        return True
...
orcid = OrcidTelescope(dag_id='orcid_test', queue='default', schedule_interval='@weekly',
                       ...
                       max_retries=3, description='', dataset_id='dataset_id', schema_version='', airflow_vars=[],
                       airflow_conns=[], download_ext='json', extract_ext='json', transform_ext='jsonl',
                       main_table_id='main_table_id', partition_table_id='partition_table_id',
                       merge_partition_field='merge_field', updated_date_field='update_field', bq_merge_days=1)
